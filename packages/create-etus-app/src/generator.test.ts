...
describe('Generator', () => {
  ...
  it('includes r2_buckets when storage module selected', async () => {
    const options: CLIOptions = {
      projectName: 'my-saas',
      domain: 'mysaas.com',
      modules: ['storage'],
    ...
    expect(wrangler.r2_buckets[0].bucket_name).toBe('my-saas-files')
  })

  it('excludes r2_buckets when storage module not selected', async () => {
    const options: CLIOptions = {
      projectName: 'my-saas',
      domain: 'mysaas.com',
      modules: [],
