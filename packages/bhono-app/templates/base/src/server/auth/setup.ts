...
export function getAuth(env: Env): AuthInstance {
  ...
  cached = createAuth({
    ...
    // @etus/auth's account routes currently treat membership role "admin" as
    // the account administrator. Product-level owner/admin/member/guest stays
    // on auth_users.role and the permission matrix; account membership writes
    // are normalized by auth/package-compat.ts until the package exposes a
    // dedicated multiTenant.roles/defaultRole contract.
    multiTenant: { enabled: true, creatorRole: 'admin' },
