...
describe('Settings Page', () => {
  ...
  describe('NotificationToggle component', () => {
    ...
    it('should have disabled Security Alerts toggle', async () => {
      const user = userEvent.setup()
      renderRoute({ initialEntries: ['/settings'] })
      ...
      const toggles = screen.getAllByRole('switch')
      const securityAlertsToggle = toggles[2] // Third toggle is Security Alerts

      expect(securityAlertsToggle).toBeDisabled()
    })

    it('should not toggle state when disabled toggle is clicked', async () => {
      const user = userEvent.setup()
      renderRoute({ initialEntries: ['/settings'] })
      ...
      const toggles = screen.getAllByRole('switch')
      const securityAlertsToggle = toggles[2]

      // Should be checked and disabled
      expect(securityAlertsToggle).toHaveAttribute('aria-checked', 'true')
      expect(securityAlertsToggle).toBeDisabled()

      // Try to click - state should remain unchanged
      await user.click(securityAlertsToggle)
      expect(securityAlertsToggle).toHaveAttribute('aria-checked', 'true')
    })
  })
})
