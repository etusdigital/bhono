...
**Auditoria de fidelidade da sidebar/topbar (2026-06-17):** todas as 11 telas conferidas contra o showcase publicado — UI 100% Seven, sem drift. **1 desvio cosmético corrigido** (brand mark do 404/invite/landing → plate mint, igual login/sidebar). **1 achado upstream:** o item ativo da sidebar aparece com fundo **mint forte** (`--sidebar-accent`) vs o **cinza sutil** do Figma (`--sidebar-nav-item-bg-hover`) — **não é defeito do boilerplate**, é o comportamento real do componente `SidebarMenuButton`. Causa raiz = **bug no Seven**: a estilização Figma (`group-data-[mode]:`) só vale no dual mode; o render single não expõe `data-mode` no `group` div. Fix do `data-mode` aplicado no working tree do Seven (ativa ícone 16→20px / padding / gap / fonte); resíduo de cascata no active-bg entregue ao time. Writeup: `seven/docs/audits/sidebar-single-mode-styling-bug.md`.
